<template>
	<view>
		...
		<!-- 底部输入栏 -->
		<view v-show="isMatch" class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			<!-- H5下不能录音，输入栏布局改动一下 -->
			<!-- #ifndef H5 -->
			...
			<view class="more" @tap="setDisconnect"><view class="setDisconnect">离开</view></view>
		...
		<view class="chonglian">
			<button v-show="!isMatch" class="Matching" type="default" @click="Matching">匹配</button>

			<!-- <button type="default" @click="setDisconnect">离开</button> -->
		</view>
	</view>
</template>
<script>
export default {
	...
	created() {
		...
		// 刷新
		//1.是否已经匹配
		if (this.$getStorageSync('isMatch') === true) {
			this.isMatch = true;
			// 2.myuid写上
			this.myuid = this.$getStorageSync('myuid');
			// 2.msglist写上
			this.getMsgList();
			// this.msgList=this.$getStorageSync('msgList');
		} else {
			this.isMatch = false;
		}
	...
	sockets: {
		...
		connect() {
			console.log('新连接');
			// 1.如果存在id，表示是重连，给服务器新id和旧id，新id替换旧id
			if(this.$getStorageSync('myuid')){
				this.$socket.emit('newConnection', this.$getStorageSync('myuid'), this.$socket.id);
			}

			//2.存id到data
			this.myuid = this.$socket.id;

			// 3.存id到缓存
			try {
				uni.setStorageSync('myuid', this.myuid);
			} catch (e) {
				// error
			}
			console.log(this.myuid, '连接成功');
		},

		reconnect() {
			console.log('重新连接事件', socket.id);
		},

		// 连接断开事件，刷新没有触发
		disconnect() {
			console.log('连接断开', this.msgList);
		},
		//接收服务器消息
		message(obj) {
			console.log('发送消息msglist：', this.msgList);
			// console.log(obj);
			...
			let msg = {
				...
				msg: {
					...
					time: nowDate.getHours() + ':' + nowDate.getMinutes(),
					type: obj.type,
					userinfo: { uid: obj.uid, username: '大黑哥', face },
					content: obj.content
				}
			};

			// console.log(this.myuid,'发出的id');
			// 发送消息
			this.screenMsg(msg);
		},
		...
		cancelWait(msg) {
			...
			this.isMatch = true;
			try {
				uni.setStorageSync('isMatch', true);
			} catch (e) {
		...
		goAway(msg) {
			...
			// 是否匹配=否
			this.isMatch = false;
			uni.setStorageSync('isMatch', false);
		}
	},
	...
	methods: {
		...
		beforeunloadFn(e){
			...
			try {
				...
				// 是否匹配
				// uni.setStorageSync('isMatch', this.isMatch);
				// uni.setStorageSync('myuid', this.myuid);
			} catch (e) {
				// error
			}
			...
			uni.showModal({
				...
				success: res => {
					if (res.confirm) {
						this.isMatch = false;
						uni.setStorageSync('isMatch', false);
						this.$socket.emit('setDisconnect', this.myuid);
					} else if (res.cancel) {
						console.log('用户点击取消');
					}
				}
		...
		//匹配请求
		Matching() {
			// 1.断开连接.?有时候服务器没这个人.to报错
			// this.$socket.emit('setDisconnect', this.myuid);
			// 2.重新匹配
			this.$socket.emit('Matching', this.$getStorageSync('myuid'));
		...
		getMsgList() {
			...
			//判读缓存是不是数组
			let list;
			if (this.$getStorageSync('msgList') instanceof Array) {
				list = this.$getStorageSync('msgList');
			} else {
				list = [];
			}

			console.log('获取缓存', list);
			// 获取消息中的图片,并处理显示尺寸
			for (let i = 0; i < list.length; i++) {
				if (list[i].type == 'user' && list[i].msg.type == 'img') {
					list[i].msg.content = this.setPicSize(list[i].msg.content);
		...
		sendMsg(content, type) {
			...
			let lastid = this.msgList[this.msgList.length-1].msg.id;
			lastid++;
			let msg = {type:'user',msg:{id:lastid,time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,userinfo:{uid:this.myuid,username:"大黑哥",face:"/static/img/im/face/face_2.jpg"},content:content}}
			...
			this.$socket.emit('sendMsg', { content, type, uid: this.myuid });
...
.Matching {
	background-color: rgb(254, 83, 81);
	color: #ffffff;
	font-size: 28rpx;
	width: 80%;
	position: fixed;
